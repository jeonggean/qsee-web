...
$id = $_GET['ID_Laporan'];
// APPROVED
if (isset($_POST["kirim-approved"])) {
    $statuses = $_POST["statuses"];
    $queryInsert = mysqli_query($connect, "UPDATE approved SET statuses = '$statuses' WHERE ID_Laporan = '$id'");
    $_SESSION['popup'] = "Data telah tersimpan!";
    header("Location: ../manajer/Masuk-Manajer-app.php");
}

// PERBAIKAN
if (isset($_POST["kirim-perbaikan"])) {
    $evaluasi  = $_POST["evaluasi"];
    $rekomendasi_perbaikan  = $_POST["rekomendasi_perbaikan"];
    if (empty($evaluasi) || empty($rekomendasi_perbaikan)) {
        $error = "Lengkapi semua kolom yang tersedia.";
    } else {
        $queryInsert = mysqli_query($connect, "UPDATE perbaikan SET evaluasi = '$evaluasi', rekomendasi_perbaikan = '$rekomendasi_perbaikan' WHERE ID_Laporan = '$id'");
        $_SESSION['popup'] = "Data telah tersimpan!";
        header("Location: ../manajer/Masuk-Manajer-per.php");
    }
}

// PEMBUANGAN
if (isset($_POST["kirim-pembuangan"])) {
    $evaluasi  = $_POST["evaluasi"];

    if (empty($evaluasi)) {
        $error = "Lengkapi semua kolom yang tersedia.";
    } else {
        $queryInsert = mysqli_query($connect, "UPDATE pembuangan SET evaluasi = '$evaluasi' WHERE ID_Laporan = '$id'");
        $_SESSION['popup'] = "Data telah tersimpan!";
        header("Location: ../manajer/Masuk-Manajer-pem.php");
    }
}
?>
...
<head>
  ...
  <link rel="stylesheet" href="Hasil-Keluar.css">
</head>
<body>
  <aside>
    <div>
          ...
          <!-- Pemeriksaan Fisik -->
                ...
                ?>
                  ...
                  <form action="" method="POST">
                    <input type="hidden" value="Terbaca" name="statuses">
                    <button class="submit-btn" name="kirim-approved" type="submit">✅CHECKLIST</button>
                  </form>
                <?php
                  ...
                  <form action="" method="POST">
                    ...
                    <div class="box">
                        ...
                        <input type="textarea" name="evaluasi">
                    </div>
                    <div class="box">
                        <div class="box-title">REKOMENDASI PERBAIKAN</div>
                        <input type="textarea" name="rekomendasi_perbaikan">
                    </div>
                  <button class="submit-btn" name = "kirim-perbaikan" type="submit">SUBMIT</button>
                  </form>
                <?php
                  ...
                  <button class="submit-btn" name = "kirim-pembuangan" type="submit">SUBMIT</button>
                  </form>
                <?php
                } ?>
