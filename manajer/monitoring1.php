...
<main>
    <section class="dashboard-cards">
        ...
        <?php
        include '../global/connect.php'; // Sesuaikan path
        ...
        foreach ($tahunList as $tahun) {
            ...
            for ($bulan = 1; $bulan <= 12; $bulan++) {
                $approved = mysqli_fetch_assoc(mysqli_query($connect,
                    "SELECT COUNT(*) AS total FROM kualitas_produk 
                    WHERE status = 'approved' AND MONTH(Tanggal_produksi) = $bulan AND YEAR(Tanggal_produksi) = $tahun"
                ))['total'];

                $perbaikan = mysqli_fetch_assoc(mysqli_query($connect,
                    "SELECT COUNT(*) AS total FROM kualitas_produk 
                    WHERE status = 'perbaikan' AND MONTH(Tanggal_produksi) = $bulan AND YEAR(Tanggal_produksi) = $tahun"
                ))['total'];

                $pembuangan = mysqli_fetch_assoc(mysqli_query($connect,
                    "SELECT COUNT(*) AS total FROM kualitas_produk 
                    WHERE status = 'pembuangan' AND MONTH(Tanggal_produksi) = $bulan AND YEAR(Tanggal_produksi) = $tahun"
                ))['total'];
